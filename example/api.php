...
case 'vacuum':
  ...
  $pdo->query('PRAGMA legacy_alter_table=ON');
...
try {
  ...
  if ($assoc) {
    ...
    while ($assoc = $stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      foreach ($assoc as $key => $val) {
        //echo $comma.'"'.$val.'"';
        //echo $comma.json_encode($val, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_APOS);
        //$s = json_encode($val, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_QUOT);
        //if ()
        echo $comma.json_encode($val, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_QUOT);
        $comma = ",";
      }
      echo ']';
    }
